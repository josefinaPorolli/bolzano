...
class BolzanoApp:
    ...
    def _plot_funcion(self, a=None, b=None):
        ...
        if a is None or b is None:
            ...
            RANGO_DEFECTO = 10.0  # cuánto mostrar a cada lado cuando el dominio es infinito
            ...
            try:
                ...
                # Dominio no acotado (ej. 1/x -> (-oo,0) U (0,oo)): los
                # infinitos no sirven como límites de graficación, así que
                # se reemplazan por un rango finito razonable alrededor del
                # origen / de las exclusiones conocidas.
                if not np.isfinite(a_v):
                    a_v = -RANGO_DEFECTO
                if not np.isfinite(b_v):
                    b_v = RANGO_DEFECTO
        ...
        for x in xs:
            try:
                ...
                if y is None:
                    raise ValueError
                y = float(y)
                xs_raw.append(float(x))
                ys_raw.append(y if np.isfinite(y) else np.nan)
            except Exception:
                xs_raw.append(float(x))
                ys_raw.append(np.nan)

        xs_arr = np.array(xs_raw, dtype=float)
        ys_arr = np.array(ys_raw, dtype=float)

        # ------------------------------------------------------------
        # Rango Y "razonable" con percentiles: usa el intervalo que
        # cubre del percentil 2 al 98 de los valores finitos. Esto se
        # adapta solo al tipo de función: en una función acotada (sin,
        # polinomios, etc.) ese rango ya cubre casi toda la amplitud
        # real, así que la curva ocupa el alto disponible. En una
        # función con asíntota, el pequeño porcentaje de puntos que se
        # dispara cerca de la singularidad queda excluido del cálculo.
        # ------------------------------------------------------------
        finitos = ys_arr[np.isfinite(ys_arr)]
        if finitos.size == 0:
            y_lo, y_hi = -10.0, 10.0
        else:
            p_lo, p_hi = np.percentile(finitos, [2, 98])
            rango = p_hi - p_lo
            if rango <= 0:
                rango = abs(p_hi) if p_hi != 0 else 1.0
            y_lo, y_hi = p_lo, p_hi
            # aseguramos que el 0 quede visible si la función lo cruza
            if y_lo > 0:
                y_lo = -rango * 0.1
            if y_hi < 0:
                y_hi = rango * 0.1

        margen_y = (y_hi - y_lo) * 0.12
        y_lo -= margen_y
        y_hi += margen_y

        # ------------------------------------------------------------
        # Reconstrucción de la serie a graficar: cuando un tramo se va
        # del rango visible, se agrega UN punto tocando el borde (para
        # que la curva "llegue" hasta ahí) y se corta con NaN. Los
        # demás puntos fuera de rango se descartan sin dibujarlos.
        # ------------------------------------------------------------
        xs_f, ys_f = [], []
        dentro_prev = True
        for x, y in zip(xs_arr, ys_arr):
            ...
            fuera = y < y_lo or y > y_hi
            if fuera:
                if dentro_prev:
                    borde = y_hi if y > 0 else y_lo
                    xs_f.append(x)
                    ys_f.append(borde)
                    xs_f.append(x)
                    ys_f.append(np.nan)
